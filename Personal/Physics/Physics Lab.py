...
while True:
    ...
    for O in objects:
        ...
        if O.attached:
            ...
            screen.blit(pin,(stringpin.center[0]-2.5,stringpin.center[1]-2.5))
            forces.append((vectorcomponent(angleturn(O,stringpin),hookeslaw(O,stringpin),"x"),(vectorcomponent(angleturn(O,stringpin),hookeslaw(O,stringpin),"y"))))
            pygame.draw.line(void,"White",O.center,stringpin.center,2)
        window_blit(O)
        for L in liquids:
            if O.rect.colliderect(L.rect):
                forces.append((0,Archimedes(O,L)))
                forces.append((O.velocity[0]*-0.1,O.velocity[1]*-0.1))
                k = 0
        for Oi in objects:
            for F in floors:
                ...
                if O.rect.colliderect(Oi.rect) and O != Oi and not Oi.fixed and not O.fixed:
                    ...
                    if Oi.absX < O.absX and abs(Oi.absX - O.absX) > abs(Oi.absY - O.absY):
                        ...
                        Oi.absX = O.absX + O.surf.get_width()
                        O.absX = Oi.absX - Oi.surf.get_width()
                    print(((3*system_force)/4)/Oi.mass)
