...
while play == True:
    ...
    while pycoderun == True:
        while stage == 0:
            ...
            TowersDownangle = []
            TowersDownturntype = []
            Towers = [swordsman,sniper,builder,medusa,bomber,midas,wizard,boxer]
            Towersrange = [100,100000000,100000000,100,200,0,125,85]
            Towerspriority = ["Close","Strong","Close","Close","Strong","Close","Strong","Close"]
            Towersidle = [swordsmanidle,sniperidle,builderidle,medusaidle,bomberidle,midasidle,wizardidle,boxeridle]
            Towersicon = [swordsmanicon_r,snipericon_r,buildericon_r,medusaicon_r,bombericon_r,midasicon_r,wizardicon_r,boxericon_r,rockstaricon_r,spidericon_r,blackwhiteicon_r]
            animationtimer = [1,5,3,0,5,10,1,0.1] 
            Towersappendage = [sword,rifle,trowel,False,blank,False,staff,bothfist]
            TDanimationtimer = []
            TDanimationslide = []
            Towersturntype = ["Full","Full","None","None","None","None","Full","Full"]
            TDdmgmult = []
                ...
                for x in range(len(enemy)):
                    ...
                    if not len(enemy) - y <= 0:
                        if enemyhealth[y] <= 0:
                                ...
                                del ycords[y]
                                ##print(len(enemy))
                                for y in range(len(midascords)):
                                    if math.sqrt((Towerscords[killernum][0]-midascords[y][0])**2 + ((Towerscords[killernum][1]-midascords[y][1])**2)) <= 200:
