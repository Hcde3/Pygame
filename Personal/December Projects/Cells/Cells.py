    ...
    while play:
        ...
        if selectedmap == "split":
            ...
            if pellettimer < 0:
                ...
                pellettimer -= 1/tick
            if budtimer < 0 and len(bud_L) < 100 and global_timer >= 0:
                circular = False
                while not circular:
                    print("here")
            ...
            if budtimer < 0 and len(bud_L) < 100 and global_timer >= 0:
                ...
                while not circular:
                    ...
                    if math.sqrt((n_x-(window_sz/2))**2 + (n_y-(window_sz/2))**2) <= window_sz-100 and math.sqrt((n_x-(window_sz/2))**2 + (n_y-(window_sz/2))**2) >= window_sz-200:
                        bud_L.append(Bud(n_x,n_y, bud, bud.get_rect(center = (n_x,n_y)), 8))
                        circular = True
                    else:
                        circular = False
